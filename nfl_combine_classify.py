...
class nflCombineClassify(nflCombineRegressor):
    ...
    def snaps_to_binary(self):
        ...
        for df, name in [(self.pd_2013, "pd_2013"), (self.pd_2014, "pd_2014"), (self.pd_2015, "pd_2015"),
                         (self.pd_2016, "pd_2016"), (self.pd_2017, "pd_2017"),
                         (self.new_pd_2013, "new_pd_2013"), (self.new_pd_2014, "new_pd_2014"),
                         (self.new_pd_2015, "new_pd_2015"), (self.new_pd_2016, "new_pd_2016"),
                         (self.new_pd_2017, "new_pd_2017")]:
        ...
        # binary: did record ≥1 solo tackle?
        for df in [merged_2013, merged_2014, merged_2015, merged_2016, merged_2017]:
            df['TARGET'] = (df['TARGET'] > 0).astype(int)
        
        X_2013 = merged_2013[cols]; y_2013 = merged_2013['TARGET']
        X_2014 = merged_2014[cols]; y_2014 = merged_2014['TARGET']
        X_2015 = merged_2015[cols]; y_2015 = merged_2015['TARGET']
        X_2016 = merged_2016[cols]; y_2016 = merged_2016['TARGET']
        X_2017 = merged_2017[cols]; y_2017 = merged_2017['TARGET']
    ...
    def model_test_classify(self):
        ...
        max_depth = [int(x) for x in np.linspace(1, 110, num=10)] + [None]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run NFL Combine Classifier with merged combine data (old files) and binary SOLO‑tackle TARGET."
    )
    parser.add_argument(
        "--path", 
        type=str, 
        default="", 
        help="Folder path containing NFL_20xx_edit.xlsx files and new data files."
    )
