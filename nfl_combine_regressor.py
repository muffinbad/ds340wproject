...
class nflCombineRegressor:
    ...
    def load_new_data(self):
        # only change here: read `Solo` instead of RUTD+RECTD
        ...
        for df in [self.new_pd_2013, self.new_pd_2014, self.new_pd_2015, self.new_pd_2016, self.new_pd_2017]:
            ...
            # HERE: parse Solo column, make it the TARGET
    ...
    def split_test(self):
        ...
        for df, name in [
            (self.pd_2013, "pd_2013"), (self.pd_2014, "pd_2014"), (self.pd_2015, "pd_2015"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run NFL Combine Regressor merging old combine data with new SOLO‑tackle TARGET."
    )
    parser.add_argument(
        "--path", 
        type=str, 
